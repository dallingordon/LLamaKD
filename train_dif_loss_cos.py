...
def train_model(modelname, kwargsfile, epochs, datasetname,datafolder, savename, lr, bs, resume, clip, he, CE, ceweight, SLD):
    ...
    if hasattr(kd_data_sets, datasetname):
        ...
        dataloader = DataLoader(dataset, batch_size=bs, shuffle=True,num_workers=4,prefetch_factor=4) #also here this is speedy
    else:
    ...
    criterion_sld = CosineDistanceLoss()
    ...
    for epoch in range(epochs):
        ...
        for data, labels in dataloader:
            ...
            outputs = model(data)
            combined_loss = criterion(outputs, labels)
            mse = combined_loss.item() 
            ...
            if CE:
                ...
                combined_loss += ceweight*loss_ce #ceweight is optional now.

            if SLD:
                #this Shuffled Label Differences.  
                shuffled_labels = labels[torch.randperm(labels.size(0))]
                label_targets = (labels - shuffled_labels) ##you goose, this cancels in mse
                label_logits = (outputs - shuffled_labels)
                loss_sld = criterion_sld(label_logits, label_targets)
                combined_loss += loss_sld ##this should probs have a weight too.
         ...
         # This assumes that your loss is already MSE
        ...
        if SLD:
            sldl = loss_sld.item()
            print(f"\tShuffled Label Diff Cosine Distance: {sldl:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr", type=float, default=0.001, help="Learning rate (defaults to 0.001)")
    parser.add_argument("--bs", type=int, default=50, help="batch size (defaults to 50)")
    ...
    parser.add_argument("--SLD", action="store_true", help="performs Shuffled Label Differences MSE")
    args = parser.parse_args()



    train_model(args.modelname, args.kwargsfile, args.epochs, args.datasetname,args.datafolder, args.savename, args.lr, args.bs, args.resume, args.clip, args.he, args.CE, args.ceweight, args.SLD)
