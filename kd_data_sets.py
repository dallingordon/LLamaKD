    ...
    def __init__(self, data_dir):
        ...
        for file_name in all_files:
            if file_name.startswith("input") and file_name.endswith(".pt"): #looks for formatted pt files. 
                ...
                # Check if there is a corresponding target file
                if target_file_name in all_files:
                    if int(file_id) < 5000:  # Adjust this condition as needed for your subset #remove for real thing
                        # print(file_id)
                        self.file_ids.append(int(file_id)) #appends only if there is a target and input file
                        if file_size == 0:
                            input_file_path = os.path.join(data_dir, f'input{file_id}.pt')
...
class IdxDataset(Dataset):
    def __init__(self, data_dir):
        ...
        for id in file_ids:
            ...
            # Load input and target tensors
            input_tensor = torch.load(input_file_path).to(torch.int64).to('cpu')
            target_tensor = torch.load(target_file_path).to('cpu')
    ...
    def __getitem__(self, idx):
        oh_input = torch.zeros(1,512,32_000)
        
        input_indexes = self.combined_input_dataset[idx][0]

        oh_input.scatter_(2, input_indexes.unsqueeze(-1).unsqueeze(0), 1)
        oh_input = oh_input.squeeze()
        return (
            oh_input,
            self.combined_target_dataset[idx][0]  # Get the target tensor
        )
    
class OHIdxDataset(Dataset):
    def __init__(self, data_dir):
        self.data_dir = data_dir
        self.input_datasets = []
        self.target_datasets = []
        ...
        for id in file_ids:
            ...
            # Load input and target tensors
            input_tensor = torch.load(input_file_path).to(torch.int64)
            target_tensor = torch.load(target_file_path)
    ...
    def __getitem__(self, idx):
        oh_input = torch.zeros(1,512,32_000)
        oh_output = torch.zeros(32_000)
        input_indexes = self.combined_input_dataset[idx][0]
        #print(input_indexes.shape)
        oh_input = oh_input.to(input_indexes.device)
        #print(oh_input.shape, "input")
        oh_input.scatter_(2, input_indexes.unsqueeze(-1).unsqueeze(0), 1)

        out_id = torch.argmax(self.combined_target_dataset[idx][0])
        oh_output[out_id] = 1.0
        #print(oh_output, oh_output.shape, "outputs")
        return (
            oh_input,
              oh_output
        )
...
class TestDFDS(Dataset): #could make variable file?
    def __init__(self, data_dir):
        self.data_dir = data_dir
        self.file_ids = []
        self.total_length = 0  # Initialize the total length
        ...
        for file_name in all_files:
            if file_name.startswith("input") and file_name.endswith(".pt"): #looks for formatted pt files. 
                ...
                # Check if there is a corresponding target file
                if target_file_name in all_files:
                    if int(file_id) < 10:  # Adjust this condition as needed for your subset #remove for real thing
                        # print(file_id)
                        self.file_ids.append(int(file_id)) #appends only if there is a target and input file
                        if file_size == 0:
                            input_file_path = os.path.join(data_dir, f'input{file_id}.pt')
